    ...
    private func prepareAppBundle(for targetAppBundle: ALTApplication, profiles: [String: ALTProvisioningProfile], appexBundleIds: [String: String]) async throws -> URL {
        ...
        if targetAppBundle.isAltStoreApp {
            ...
            do {
                ...
                var fetched = try? await fetchUDID()
                if fetched == nil || fetched?.isEmpty == true {
                    fetched = try? await fetchUDID(useStatic: true)
                }
                guard let validUdid = fetched, !validUdid.isEmpty else { throw OperationError.unknownUDID }
                udid = validUdid
            } catch {
            ...
            var fetchedUdid = try? await fetchUDID()
            if fetchedUdid == nil || fetchedUdid?.isEmpty == true {
                fetchedUdid = try? await fetchUDID(useStatic: true)
            }
            if let udid = fetchedUdid, !udid.isEmpty {
